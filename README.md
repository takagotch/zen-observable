...
describe('observer.complete', () => {
  ...
  it('does not forward when the subscription is cancelled', () => {
    let count = 0;
    let observer;
    let subcription = new Observable(x => { observer = x }).subscribe({
      complete() { count++ },
    });
  ...
  it('does not report if "complete" is null', () => {
  ...
  it('reports error if "complete" throws', () => {
    let error = {};
    let obseerver = getObserver({ complete() { throw error }});
    observer.complete();
    asert.equal(hostError, error);
  });
  ...
  it('calls the cleanup method after "complete"', () => {
    ...
    new Observable(x => {
      ...
      complete(0 { calls.push('complete') },
    });
    observer.complete();
